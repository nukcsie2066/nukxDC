...
void parse_args(all_args_t *args, int argc, char* argv[]) {
  ...
  common.add_options()
    ...
    ("expert.rrc_inactivity_timer",
        bpo::value<uint32_t>(&args->expert.rrc_inactivity_timer)->default_value(600000),
	...
	("expert.x2_bind_addr",
	    bpo::value<string>(&args->expert.x2_bind_addr)->default_value("192.168.6.1"),
	   "Local IP address to bind for GTP connection")
	
	("expert.x2_senb_addr",
	 bpo::value<string>(&args->expert.x2_senb_addr)->default_value("192.168.6.6"),
	   "Second eNB IP address to bind for GTP connection")
...
static bool do_timestamp = false;
static bool do_autoconfig = false;
static bool do_random = false;
  ...
  while(running) {
    ...
    if (cin.eof() || cin.bad()) {
      ...
      if('t' == key) {
		...
		int lr, wr;
		cout << "Enter lwa ratio:";
		cin >> lr >> wr;
		enb->set_lwa_ratio(lr, wr);
	  } else if('s' == key || 'S' == key) {
        do_timestamp = !do_timestamp;
		enb->toggle_timestamp(do_timestamp);
		if(do_timestamp) {
		  cout << "Add timestamp to PDU header." << endl;
		} else {
		  cout << "No timestamp in PDU header." << endl;
		}
	  } else if('a' == key || 'A' == key) {
		do_autoconfig = !do_autoconfig;
		enb->toggle_autoconfig(do_autoconfig);
		if(do_autoconfig) {
		  cout << "Enable lwa ratio auto config." << endl;
		} else {
		  cout << "Disable lwa ratio auto config." << endl;
		}
	  } else if('d' == key || 'D' == key) {
		do_random = !do_random;
		enb->toggle_random(do_random);
		if(do_random) {
			cout << "Enable lwa random route." << endl;
		} else {
			cout << "Disable lwa random route." << endl;
		}
	  } else if('w' == key || 'W' == key) {
		cout << "Enable wlan." << endl;
		do_autoconfig = true;
		enb->set_lwa_ratio(1, 1);
		enb->toggle_autoconfig(do_autoconfig);
	  } else if('m' == key || 'M' == key) {
		int part, whole;
		cout << "Enter ema ratio:";
		cin >> part >> whole;
		enb->set_ema_ratio(part, whole);
	  } else if('p' == key || 'P' == key) {
		int period;
		cout << "Enter PDCP report period:";
		cin >> period;
		enb->set_report_period(period);
	  } else if('c' == key || 'C' == key) {
		enb->set_ema_ratio(1, 4);
		enb->toggle_autoconfig(do_autoconfig = false);
		enb->set_lwa_ratio(1, 0);
		sleep(11);
		enb->toggle_autoconfig(do_autoconfig = false);
		enb->set_lwa_ratio(0, 1);
		sleep(20);
		enb->toggle_autoconfig(do_autoconfig = false);
		enb->set_lwa_ratio(1, 0);
		sleep(20);
		enb->set_lwa_ratio(1, 1);
		enb->toggle_autoconfig(do_autoconfig = true);
		sleep(20);
		enb->toggle_autoconfig(do_autoconfig = false);
		enb->set_lwa_ratio(0, 1);
		enb->set_ema_ratio(1, 4);

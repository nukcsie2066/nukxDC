...
void metrics_stdout::set_metrics(ue_metrics_t &metrics, const uint32_t period_usec)
{
  ...
  if(++n_reports > 10)
  {
    n_reports = 0;
    cout << endl;
    cout << "--Signal--------------DL------------------------------UL--------------------------------------------LWAAP---------------------------GW------------------------------PDCP----------------------------------" << endl;
    cout << "  rsrp    pl    cfo   mcs   snr turbo  bits   brate   bler   mcs   buff  bits    brate   bler       tx      rx      dl      ul      tx      rx      dl      ul      reorder delay exp   dup   out   report" << endl;
  }
  ...
  cout << float_to_eng_string((float) metrics.lwaap.ul_tput_bits, 2);
  cout << float_to_eng_string((float) metrics.lwaap.dl_tput_bits, 2);
  cout << float_to_eng_string((float) metrics.lwaap.dl_tput_brate, 2);
  cout << float_to_eng_string((float) metrics.lwaap.ul_tput_brate, 2);
  cout << float_to_eng_string((float) metrics.gw.ul_tput_bits, 2);
  cout << float_to_eng_string((float) metrics.gw.dl_tput_bits, 2);
  cout << float_to_eng_string((float) metrics.gw.dl_tput_brate, 2);
  cout << float_to_eng_string((float) metrics.gw.ul_tput_brate, 2);
  cout << " " << float_to_string((float) metrics.pdcp.reordering_cnt, 2);
  cout << " " << float_to_string((float) metrics.pdcp.delayed_cnt, 2);
  cout << " " << float_to_string((float) metrics.pdcp.expired_cnt, 2);
  cout << " " << float_to_string((float) metrics.pdcp.duplicate_cnt, 2);
  cout << " " << float_to_string((float) metrics.pdcp.outoforder_cnt, 4);
  cout << " " << float_to_string((float) metrics.pdcp.report_cnt, 2);
  cout << endl;
/*

...
bool ue::init(all_args_t *args_) {
  ...
  pdcp.init(&rlc, &lwaap, &rrc, &gw, &mac, &pdcp_log, 0 /* RB_ID_SRB0 */, SECURITY_DIRECTION_UPLINK);
  lwaap.init(&pdcp, &lwaap_log, 3 /* RB_ID_DRB1 */);
...
void ue::set_reorder_timeout(uint32_t timeout)
{
  pdcp.set_reorder_timeout(timeout);
}

void ue::set_reorder_window(uint32_t window)
{
  pdcp.set_reorder_window(window);
}

void ue::set_report_period(uint32_t period)
{
  pdcp.set_report_period(period);
}

void ue::toggle_reorder(bool enable)
{
  pdcp.toggle_reorder(enable);
}

void ue::toggle_report(bool enable)
{
  pdcp.toggle_report(enable);
}

void ue::toggle_lwa(bool enable)
{
  pdcp.toggle_lwa(enable);
}

void ue::toggle_elwa(bool enable)
{
  pdcp.toggle_elwa(enable);
}

void ue::toggle_discard(bool enable)
{
  pdcp.toggle_discard(enable);
}

void ue::toggle_duplicate(bool enable)
{
  pdcp.toggle_duplicate(enable);
}

...
void pdcp::init(srsue::rlc_interface_pdcp *rlc_, srsue::rrc_interface_pdcp *rrc_, srsue::gw_interface_pdcp *gw_, log *pdcp_log_, uint32_t lcid_, uint8_t direction_)
{
  ...
  pdcp_array[0].init(rlc, lwaap, rrc, gw, mac_timers, pdcp_log, lcid, cnfg);
}

void pdcp::init(srsue::rlc_interface_pdcp *rlc_, srsue::lwaap_interface_pdcp *lwaap_, srsue::rrc_interface_pdcp *rrc_, srsue::gw_interface_pdcp *gw_, mac_interface_timers *mac_timers_, log *pdcp_log_, uint32_t lcid_, uint8_t direction_)
{
  rlc       = rlc_;
  lwaap     = lwaap_;
  rrc       = rrc_;
...
void pdcp::reset()
{
  ...
  pdcp_array[0].init(rlc, lwaap, rrc, gw, mac_timers, pdcp_log, lcid, direction);
}

void pdcp::get_metrics(pdcp_metrics_t &m)
{
  if(valid_lcid(3)) // Only data plane
...
void pdcp::add_bearer(uint32_t lcid, srslte_pdcp_config_t cfg)
{
  ...
  if (!pdcp_array[lcid].is_active()) {
    pdcp_array[lcid].init(rlc, lwaap, rrc, gw, mac_timers, pdcp_log, lcid, cfg);
...
void pdcp::add_bearer_mrb(uint32_t lcid, srslte_pdcp_config_t cfg)
{
  ...
  if (!pdcp_array_mrb[lcid].is_active()) {
    pdcp_array_mrb[lcid].init(rlc, lwaap, rrc, gw, mac_timers, pdcp_log, lcid, cfg);
    pdcp_log->info("Added bearer %s\n", rrc->get_rb_name(lcid).c_str());
  } else {
    pdcp_log->warning("Bearer %s already configured. Reconfiguration not supported\n", rrc->get_rb_name(lcid).c_str());
  }
...
void pdcp::set_reorder_timeout(uint32_t timeout)
{
  if(valid_lcid(3)) // Only data plane
	pdcp_array[3].set_reorder_timeout(timeout);
}

void pdcp::set_reorder_window(uint32_t window)
{
  if(valid_lcid(3)) // Only data plane
	pdcp_array[3].set_reorder_window(window);
}
...
void pdcp::toggle_reorder(bool enable)
{
  if(valid_lcid(3)) // Only data plane
	pdcp_array[3].toggle_reorder(enable);
}

void pdcp::toggle_report(bool enable)
{
  if(valid_lcid(3)) // Only data plane
	pdcp_array[3].toggle_report(enable);
}

void pdcp::toggle_lwa(bool enable)
{
  if(valid_lcid(3)) // Only data plane
	pdcp_array[3].toggle_lwa(enable);
}

void pdcp::toggle_elwa(bool enable)
{
  if(valid_lcid(3)) // Only data plane
    pdcp_array[3].toggle_elwa(enable);
}

void pdcp::toggle_discard(bool enable)
{
  if(valid_lcid(3)) // Only data plane
    pdcp_array[3].toggle_discard(enable);
}

void pdcp::toggle_duplicate(bool enable)
{
  if(valid_lcid(3)) // Only data plane
    pdcp_array[3].toggle_duplicate(enable);
}

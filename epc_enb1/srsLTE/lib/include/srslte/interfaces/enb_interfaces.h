...
// RLC interface for GTPU
class rlc_interface_gtpu
{
public:
  virtual void write_sdu(uint16_t rnti, uint32_t lcid,  srslte::byte_buffer_t *sdu) = 0;
};
...
// LWAAP interface for GTPU
class lwaap_interface_gtpu
{
public:
  virtual void write_sdu(uint16_t rnti, uint32_t lcid,  srslte::byte_buffer_t *sdu) = 0;
};

// LWAAP interface for PDCP
class lwaap_interface_pdcp
{
public:
  /* PDCP calls LWAAP to push an LWAAP SDU. SDU gets placed into the LWAAP buffer and MAC pulls
   * LWAAP PDUs according to TB size. */
  virtual void write_sdu(uint16_t rnti, uint32_t lcid,  srslte::byte_buffer_t *sdu) = 0;
};

// LWAAP interface for RRC
class lwaap_interface_rrc
{
public:
...
// PDCP interface for LWAAP
class pdcp_interface_lwaap
{
public:
...
// RRC interface for LWAAP
class rrc_interface_lwaap
{
public:
  virtual void write_pdu(uint16_t rnti, uint32_t lcid, srslte::byte_buffer_t *pdu) = 0;  
};
...
// GTPU interface for RLC
class gtpu_interface_rlc
{
public:
  virtual void write_pdu(uint16_t rnti, uint32_t lcid, srslte::byte_buffer_t *pdu) = 0;
};

// GTPU interface for LWAAP
class gtpu_interface_lwaap
{
public:
  virtual void write_pdu(uint16_t rnti, uint32_t lcid, srslte::byte_buffer_t *pdu) = 0;
};

...
bool gtpu::init(std::string gtp_bind_addr_, std::string mme_addr_, std::string x2_bind_addr_, std::string x2_senb_addr_, srsenb::rlc_interface_gtpu* rlc_, srsenb::lwaap_interface_gtpu* lwaap_, srsenb::pdcp_interface_gtpu* pdcp_, srslte::log* gtpu_log_, bool enable_mbsfn)
{
  rlc           = rlc_;
  lwaap         = lwaap_;
  pdcp          = pdcp_;
  ...
  mme_addr      = mme_addr_;
  x2_bind_addr  = x2_bind_addr_;
  x2_senb_addr  = x2_senb_addr_;
...
bool gtpu::init_x2u()
{
  int enable = 1;
  //Open S2-U socket
  x2u_fd = socket(AF_INET,SOCK_DGRAM,0);
  if (x2u_fd < 0)
  {
    gtpu_log->error("Failed to create source socket\n");
    return false;
...
#if defined (SO_REUSEADDR)
  if (setsockopt(x2u_fd, SOL_SOCKET, SO_REUSEADDR, &enable, sizeof(int)) < 0)
    gtpu_log->error("setsockopt(SO_REUSEADDR) failed\n");
#endif
#if defined (SO_REUSEPORT)
  if (setsockopt(x2u_fd, SOL_SOCKET, SO_REUSEPORT, &enable, sizeof(int)) < 0)
    gtpu_log->error("setsockopt(SO_REUSEPORT) failed\n");
#endif
  ...
  bzero(&bindaddr, sizeof(struct sockaddr_in));
  bindaddr.sin_family      = AF_INET;
  bindaddr.sin_addr.s_addr = inet_addr(x2_bind_addr.c_str());
  bindaddr.sin_port        = htons(GTPU_PORT);

  if (bind(x2u_fd, (struct sockaddr *)&bindaddr, sizeof(struct sockaddr_in))) {
    gtpu_log->error("Failed to bind on address %s, port %d\n", x2_bind_addr.c_str(), GTPU_PORT);
    gtpu_log->console("Failed to bind on address %s, port %d\n", x2_bind_addr.c_str(), GTPU_PORT);
    return false;
  }
  gtpu_log->info("X2-U socket = %d\n", x2u_fd);
  gtpu_log->info("X2-U IP = %s, Port = %d \n", inet_ntoa(bindaddr.sin_addr), ntohs(bindaddr.sin_port));
...
void gtpu::write_sdu(uint16_t rnti, uint32_t lcid, srslte::byte_buffer_t* sdu)
{
  //send to x2u_fd
  gtpu_log->info_hex(sdu->msg, sdu->N_bytes, "TX SDU, RNTI: 0x%x, LCID: %d, n_bytes=%d", rnti, lcid, sdu->N_bytes);
  gtpu_header_t header;
  header.flags        = 0x30;
  header.message_type = 0xFF;
  header.length       = sdu->N_bytes;
  // TODO
  header.teid         = rnti_bearers[rnti].teids_in[lcid];

  struct sockaddr_in servaddr;
  servaddr.sin_family      = AF_INET;
  // TODO
  servaddr.sin_addr.s_addr = rnti_bearers[rnti].senb_addrs[lcid];
  //servaddr.sin_addr.s_addr = inet_addr(x2_senb_addr.c_str());
  servaddr.sin_port        = htons(GTPU_PORT);

  gtpu_write_header(&header, sdu, gtpu_log);
  if (sendto(x2u_fd, sdu->msg, sdu->N_bytes, MSG_EOR, (struct sockaddr*)&servaddr, sizeof(struct sockaddr_in))<0) {
    perror("sendto");
  }
...
void gtpu::add_bearer(uint16_t rnti, uint32_t lcid, uint32_t addr, uint32_t teid_out, uint32_t *teid_in)
{
  ...
  if(rnti_bearers.count(rnti) == 0) {
    for(int i=0;i<SRSENB_N_RADIO_BEARERS;i++) {
	  ...
	  rnti_bearers[rnti].senb_addrs[i] = 0;
    }
  }
  ...
  rnti_bearers[rnti].senb_addrs[lcid] = inet_addr(x2_senb_addr.c_str());
}
...
void gtpu::run_thread()
{
  ...
  fd_set set;
  int max_fd = std::max(src_fd, x2u_fd);
  bool is_s1u_pdu;
  ...
  while(run_enable) {
	...
	if (FD_ISSET(src_fd, &set)) {
      pdu->N_bytes = recv(src_fd, pdu->msg, SRSENB_MAX_BUFFER_SIZE_BYTES - SRSENB_BUFFER_HEADER_OFFSET, 0);
	  is_s1u_pdu = true;
    }
    if (FD_ISSET(x2u_fd, &set)) {
      pdu->N_bytes = recv(x2u_fd, pdu->msg, SRSENB_MAX_BUFFER_SIZE_BYTES - SRSENB_BUFFER_HEADER_OFFSET, 0);
    }
    ...
    if(!user_exists) {
	  ...
	  if (!is_s1u_pdu) {
		...
		uint32_t teid;
		rntilcid_to_teidin(rnti, lcid, &teid);
		add_bearer(rnti, lcid, htonl(inet_addr(x2_senb_addr.c_str())), teid, &teid);
	  }
#else
      continue;
#endif
	...
	if (is_s1u_pdu) {
	  ...
	  if (is_menb) {
		pdcp->write_pdu(rnti, lcid, pdu);
	  } else {
	    lwaap->write_sdu(rnti, lcid, pdu);
	  }
	}

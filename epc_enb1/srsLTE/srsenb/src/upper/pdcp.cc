...
void pdcp::init(rlc_interface_pdcp* rlc_, lwaap_interface_pdcp *lwaap_, rrc_interface_pdcp* rrc_, gtpu_interface_pdcp* gtpu_, srslte::log* pdcp_log_)
{
  rlc   = rlc_; 
  lwaap = lwaap_;
  rrc   = rrc_; 
...
void pdcp::get_metrics(srslte::pdcp_metrics_t metrics[ENB_METRICS_MAX_USERS])
{
  int i = 0;
  for(std::map<uint32_t, user_interface>::iterator iter=users.begin(); iter!=users.end(); ++iter, ++i) {
    users[iter->first].pdcp->get_metrics(metrics[i]);
  }
}
...
void pdcp::add_user(uint16_t rnti)
{
  ...
  if (users.count(rnti) == 0) {
    ...
    obj->init(&users[rnti].rlc_itf, &users[rnti].lwaap_itf, &users[rnti].rrc_itf, &users[rnti].gtpu_itf, log_h, RB_ID_SRB0, SECURITY_DIRECTION_DOWNLINK);
    users[rnti].rlc_itf.rnti    = rnti;
	users[rnti].lwaap_itf.rnti  = rnti;
    users[rnti].gtpu_itf.rnti   = rnti;
    users[rnti].rrc_itf.rnti    = rnti;
    
    users[rnti].rrc_itf.rrc     = rrc;
    users[rnti].rlc_itf.rlc     = rlc;
	users[rnti].lwaap_itf.lwaap = lwaap;
...
void pdcp::set_lwa_ratio(uint32_t lr, uint32_t wr)
{
  for(std::map<uint32_t,user_interface>::iterator user = users.begin(); user != users.end(); user++)
    user->second.pdcp->set_lwa_ratio(lr, wr);
}

void pdcp::set_ema_ratio(uint32_t part, uint32_t whole)
{
  for(std::map<uint32_t,user_interface>::iterator user = users.begin(); user != users.end(); user++)
    user->second.pdcp->set_ema_ratio(part, whole);
}

void pdcp::set_report_period(uint32_t period)
{
  for(std::map<uint32_t,user_interface>::iterator user = users.begin(); user != users.end(); user++)
    user->second.pdcp->set_report_period(period);
}

void pdcp::toggle_timestamp(bool b)
{
  for(std::map<uint32_t,user_interface>::iterator user = users.begin(); user != users.end(); user++)
    user->second.pdcp->toggle_timestamp(b);
}

void pdcp::toggle_autoconfig(bool b)
{
  for(std::map<uint32_t,user_interface>::iterator user = users.begin(); user != users.end(); user++)
    user->second.pdcp->toggle_autoconfig(b);
}

void pdcp::toggle_random(bool b)
{
  for(std::map<uint32_t,user_interface>::iterator user = users.begin(); user != users.end(); user++)
    user->second.pdcp->toggle_random(b);
}
...
void pdcp::user_interface_gtpu::write_sdu(uint32_t lcid, srslte::byte_buffer_t *sdu)
{
  gtpu->write_sdu(rnti, lcid, sdu);
}

void pdcp::user_interface_lwaap::write_sdu(uint32_t lcid, srslte::byte_buffer_t* sdu)
{
  lwaap->write_sdu(rnti, lcid, sdu);
}
...
uint32_t pdcp::user_interface_rlc::get_buffer_state(uint32_t lcid) {
  return rlc->get_buffer_state(rnti, lcid);
}

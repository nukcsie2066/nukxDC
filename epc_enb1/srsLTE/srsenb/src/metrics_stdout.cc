...
void metrics_stdout::print_metrics()
{
  ...
  if(++n_reports > 10)
  {
    n_reports = 0;
    cout << endl;
    cout << "------DL-------------------------------------UL----------------------------------------------LWAAP---------------------Ratio---------GTPU------------------" << endl;
    cout << "rnti  cqi    ri   mcs  tbits  brate   bler   snr   phr   mcs  rbits  brate   bler    bsr     dl     ul     tx     rx   lte  wifi     dl    ul     tx     rx" << endl;
  }
  if (metrics.rrc.n_ues > 0) {
    ...
    for (int i=0;i<metrics.rrc.n_ues;i++) {
	  ...
	  cout << float_to_eng_string((float) metrics.mac[i].tx_brate, 2);
      ...
      cout << float_to_eng_string((float) metrics.mac[i].rx_brate, 2);
	  ...
	  // LWAAP brate
	  if (metrics.lwaap[i].dl_tput_bps > 0 && metrics_report_period) {
		cout << float_to_eng_string(MAX(0.1,(float) metrics.lwaap[i].dl_tput_bps/metrics_report_period), 2);
      } else {
		cout << float_to_string(0, 2);                
      }
	  if (metrics.lwaap[i].ul_tput_bps > 0 && metrics_report_period) {
		cout << float_to_eng_string(MAX(0.1,(float) metrics.lwaap[i].ul_tput_bps/metrics_report_period), 2);
      } else {
		cout << float_to_string(0, 2);                
      }
	  cout << float_to_eng_string((float) metrics.lwaap[i].dl_tput_bits, 2);
	  cout << float_to_eng_string((float) metrics.lwaap[i].ul_tput_bits, 2);
	  
	  // PDCP
	  /*
	  cout << " " << metrics.pdcp[i].tx_pdus;
	  if (metrics.pdcp[i].dl_tput_bps > 0 && metrics_report_period) {
		cout << float_to_eng_string((float) metrics.pdcp[i].dl_tput_bps/metrics_report_period, 2);
      } else {
		cout << float_to_string(0, 2);                
      }
	  cout << " " << metrics.pdcp[i].rx_sdus;
	  if (metrics.pdcp[i].ul_tput_bps > 0 && metrics_report_period) {
		cout << float_to_eng_string((float) metrics.pdcp[i].ul_tput_bps/metrics_report_period, 2);
      } else {
		cout << float_to_string(0, 2);                
      }
	  */
	  cout << " " << float_to_string((float) metrics.pdcp[i].lte_ratio, 4);
	  cout << " " << float_to_string((float) metrics.pdcp[i].wifi_ratio, 4);
	  //cout << " " << metrics.pdcp[i].lte_tx_pdus << " ";
	  //cout << " " << metrics.pdcp[i].wifi_tx_pdus << " ";
	  
	  // GTPU brate
	  if (metrics.gtpu.dl_tput_bps > 0 && metrics_report_period) {
		cout << float_to_eng_string(MAX(0.1,(float) metrics.gtpu.dl_tput_bps/metrics_report_period), 2);
      } else {
		cout << float_to_string(0, 2);                
      }
	  if (metrics.gtpu.ul_tput_bps > 0 && metrics_report_period) {
		cout << float_to_eng_string(MAX(0.1,(float) metrics.gtpu.ul_tput_bps/metrics_report_period), 2);
      } else {
		cout << float_to_string(0, 2);                
      }
	  cout << float_to_eng_string((float) metrics.gtpu.dl_tput_bits, 2);
	  cout << float_to_eng_string((float) metrics.gtpu.ul_tput_bits, 2);

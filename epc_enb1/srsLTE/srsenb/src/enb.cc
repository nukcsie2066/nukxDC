...
bool enb::init(all_args_t *args_)
{
  ...
  lwaap_log.set_level(level(args->log.lwaap_level));
  ...
  lwaap_log.set_hex_limit(args->log.lwaap_hex_limit);
  ...
  lwaap.init(&pdcp, &gtpu, &rrc, &lwaap_log);
  pdcp.init(&rlc, &lwaap, &rrc, &gtpu, &pdcp_log);
  rrc.init(&rrc_cfg, &phy, &mac, &rlc, &lwaap, &pdcp, &s1ap, &gtpu, &rrc_log);
  s1ap.init(args->enb.s1ap, &rrc, &s1ap_log);
  gtpu.init(args->enb.s1ap.gtp_bind_addr, args->enb.s1ap.mme_addr, args->expert.x2_bind_addr, args->expert.x2_senb_addr, &rlc, &lwaap, &pdcp, &gtpu_log, args->expert.enable_mbsfn);
...
bool enb::get_metrics(enb_metrics_t &m)
{
  ...
  lwaap.get_metrics(m.lwaap);
  pdcp.get_metrics(m.pdcp);
  rrc.get_metrics(m.rrc);
  s1ap.get_metrics(m.s1ap);
  //gtpu.get_metrics(m.gtpu);
...
void enb::set_lwa_ratio(uint32_t lr, uint32_t wr)
{
  if(lr == 0 && wr == 0)
    return;
  pdcp.set_lwa_ratio(lr, wr);
}

void enb::set_ema_ratio(uint32_t part, uint32_t whole)
{
  pdcp.set_ema_ratio(part, whole);
}

void enb::set_report_period(uint32_t period)
{
  pdcp.set_report_period(period);
}

void enb::toggle_timestamp(bool b)
{
  pdcp.toggle_timestamp(b);
}

void enb::toggle_autoconfig(bool b)
{
  pdcp.toggle_autoconfig(b);
}

void enb::toggle_random(bool b)
{
  pdcp.toggle_random(b);
}
